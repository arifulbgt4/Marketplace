...
export const catalogSearchSchema = z.object({
  ...
  sort: z.enum(["price_asc", "price_desc", "name_asc", "name_desc", "newest", "oldest"]).optional().default("newest"),
...
export function validatePublish(product: {
  status: string;
  variants: { sku: string; price: unknown }[];
  media: unknown[];
  ...
  if (!product.variants || product.variants.length === 0) {
    ...
    for (const v of product.variants) {
      ...
      if (v.price == null || Number(v.price) <= 0) errors.push("All variants must have a positive price");
    }
  }
